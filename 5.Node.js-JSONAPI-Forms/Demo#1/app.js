// Avoiding the duplicate searches:
const usernames = []
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (usernames.includes(username)) {
            alert("You already searched this user")
            return
        }
        usernames.push(username)

        // Preventing empty username
        // if (!username) {
        //     alert("Enter a username")
        //     return
        // }
        // OR just set the REQUIRED attribute on the input element in HTML file 
        // and the browser will take care of it
        
        // Fetching the users' data from GitHub
        const response = await fetch(`https://api.github.com/users/${username}`)
        
        // Handling the "user not found"
